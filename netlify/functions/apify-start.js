exports.handler = async (event) => {
  ...
  try {
    ...
    if (!token) return {
      ...
      body: JSON.stringify({ error: 'APIFY_TOKEN not configured in Netlify Environment Variables' })
    };
    ...
    if (!r.ok) return {
      ...
      body: JSON.stringify({ error: data.error?.message || 'Apify start failed', detail: JSON.stringify(data).substring(0, 200) })
    };
    return {
