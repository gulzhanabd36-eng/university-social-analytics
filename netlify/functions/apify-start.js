exports.handler = async (event) => {
  ...
  try {
    ...
    if (!actorId) return {
      statusCode: 400, headers: h,
      body: JSON.stringify({ error: 'actorId not provided' })
    };
    ...
    if (!r.ok) return {
      statusCode: 400, headers: h,
      body: JSON.stringify({ error: (data.error && data.error.message) || 'Apify start failed', detail: JSON.stringify(data).substring(0, 200) })
    };
